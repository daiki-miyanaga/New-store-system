...
class Table {
    ...
    renderRow(row, index) {
        ...
        const dataCells = this.columns.map(col => {
            ...
            if (editable) {
                const inputType = col.type === 'currency' ? 'number' : col.type || 'text';
                const step = col.type === 'currency' ? '0.01' : col.step || '';
                return `
    ...
    attachEventListeners() {
        ...
        // ソート可能ヘッダーのクリックイベント
        if (this.sortable) {
            this.element.querySelectorAll('th[data-sort]').forEach(header => {
        ...
        this.element.addEventListener('click', (e) => {
            ...
            // 選択行削除ボタン
            if (button.classList.contains('btn-delete-selected')) {
                this.deleteSelected();
            }
    ...
    updateCell(input) {
        try {
            ...
            if (!rowId || !colKey) {
                console.warn('Missing row or column data:', { rowId, colKey });
                return;
            }
            ...
            if (row) {
                // 数値型の場合は適切に変換
                const column = this.columns.find(col => col.key === colKey);
                if (column && (column.type === 'number' || column.type === 'currency')) {
                    row[colKey] = parseFloat(value) || 0;
                } else {
                    row[colKey] = value;
                }
                
                if (this.onRowUpdate && typeof this.onRowUpdate === 'function') {
                    this.onRowUpdate(row, colKey, row[colKey]);
                }
            } else {
                console.warn('Row not found:', rowId);
            }
        } catch (error) {
            console.error('Cell update failed:', error, input);
        }
    }
